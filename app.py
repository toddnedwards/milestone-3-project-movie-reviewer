...
def create_review():
    if 'user' not in session or not session['user']:
        return redirect(url_for('unauthorised'))
    if request.method == "POST":
        # Assuming 'current_user_id' is available in the
        # session or another method to identify the current user
        current_user_id = session.get("user_id")
...
# Unauthorised access for users that
# aren't logged in or have no account
